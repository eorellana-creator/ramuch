...
$sql 	= $mysql->query("UPDATE equipo_prestamo SET  estado='cancelada' WHERE token ='$token';");

$_SESSION["socio_actualizado"] = "<div class='alert alert-success' role='alert'>La solicitud se ha cancelado.</div>";
...
$sql2 	= $mysql->query("SELECT * FROM equipo_prestamo WHERE token ='$token' ;");
...
$nombre_equipo = @$result12->nombre;
 ...
 <p><a href='$ruta'><img src='$ruta/images/email_cabecera.png' border='0' alt=''/></a> 
  ...
  <strong>$nombre_usuario</strong> ($email_usuario) ha cancelado la solicitud del préstamo del equipo N°$result2->id_equipo <strong>$nombre_equipo</strong> entre las fechas <strong>$fecha1</strong> y <strong>$fecha2</strong><br><br><br>
...
$mail->addAddress("$email_usuario", "$nombre_usuario");
 ...
 $mail->Subject = 'Cancelación de solicitud de Préstamo';
 ...
 if (!$mail->send()) {
   ...
   $_SESSION["equipo_prestado"]="<div class='alert alert-danger' role='alert'>Ha ocurrido un error, por favor internte nuevamente o escríbanos a montana.uchile@gmail.com indicando el equipo y fechas de solicitud de préstamo</div>";
 } else {
   echo "Message sent!";
