...
$sqlC 	= $mysql->query("SELECT u.id_usuario, u.id_usuario, u.nombre_usuario, u.email, u.estado, p.tipo_inscripcion, p.fono, p.rut, u.referencia FROM usuario AS u LEFT JOIN perfil AS p ON u.id_usuario = p.id_usuario $busqueda ;");
...
//kop
$sql 	= $mysql->query("SELECT u.id_rol, u.id_usuario, u.nombre_usuario, u.email, u.token, p.tipo_inscripcion, u.estado, u.fecha_registro, u.web_matricula_pagada, p.fono, p.rut, u.referencia FROM usuario AS u LEFT JOIN perfil AS p ON u.id_usuario = p.id_usuario $busqueda $orderby LIMIT $inicio,$fin ;");
