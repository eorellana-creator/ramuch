...
$sql = $mysql->query("UPDATE deudas SET observacion='$observacion', estado='pagada', id_usuario = '$id_usuario', fecha_modificacion = '$hoy' WHERE token='$token';");
...
$nombre_deudor = $result->nombre_deudor;
...
$sql = $mysql->query("INSERT INTO cuenta_maestra (id_usuario_sistema, nombre_usuario_sistema, id_usuario_movimiento, nombre, fecha, tipo, sub_cuenta, glosa, observacion, medio, id_transaccion, documento_respaldo, monto, estado, token)
                     VALUES ('$id_usuario', '$nombre_usuario_sistema', '$id_usuario_deuda', '$nombre_deudor', '$fecha', 'ingreso', '$result->sub_cuenta', '$result->glosa', '$observacion', '$medio', '$result->token', '', '$result->monto', 'activo', '$token_nuevo');");
...
if($subcuenta_deuda == "matricula") {
    $sql = $mysql->query("UPDATE usuario SET web_matricula_pagada='Si' WHERE id_usuario='$id_usuario_deuda';");
}
