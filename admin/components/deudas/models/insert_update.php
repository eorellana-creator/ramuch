...
if($token == "") {
    ...
    $sql = $mysql->query("INSERT INTO deudas (id_usuario, id_usuario_deuda, nombre_deudor, sub_cuenta, fecha, monto, glosa, observacion, documento_respaldo, estado, token)
                         VALUES ('$id_usuario', '$id_participante', '$nombre', '$sub_cuenta', '$fecha', '$monto', '$glosa', '$observacion', '', 'activa', '$token_nuevo');");

    $ultimo_id = $mysql->ultimo_id(); 
    $token = $token_nuevo;
} else {
    if($token != "") {
        $sql = $mysql->query("UPDATE deudas SET id_usuario_deuda='$id_participante', nombre_deudor='$nombre', sub_cuenta='$sub_cuenta', glosa='$glosa', fecha='$fecha', monto='$monto', observacion='$observacion' WHERE token='$token';");
    }
}
