...
while($result = $mysql->f_obj($sql)) {
    ...
    if ($result->prestado_a_id_usuario > 0) {
        if ($result->fecha_devolucion > '2022-01-01') {
            $fecha_actual = strtotime(date("Y-m-d", time()));
            $fecha_entrada = strtotime($result->fecha_devolucion);
        ...
        $prestado_a = "<span $color_fecha> $result->prestado_a_nombre <br> Fecha Devolución: <br> $fecha_devolucion</span>";

        // si el estado de solicitado, no se debe mostrar el boton devolucion, en cambio debe mosntrar que falta aceptar o rechazar solicitud.
        /// buscar el id_equipo para ver si en la tabla equipo_prestamos esta con estado solicitado
        //$sqlX = $mysql->query("SELECT * FROM equipo_prestamo where id_equipo ='$result->id_equipo' and estado = 'solicitado';");
        //$resultX = $mysql->f_obj($sqlX);
        //$estado_solicitud = $resultX->estado;

        $estado_solicitud = null; // Inicializar como null por defecto
        ...
        if ($estado_solicitud == 'solicitado') {
            ...
            // Si es admin y hay extensión pendiente
            //if ($id_usuario_sesion == 1 || $result->prestado_a_id_usuario == 2287) {
                $sqlExt = $mysql->query("SELECT estado_extension, token, fecha_propuesta_extension FROM equipo_prestamo 
                                        WHERE id_equipo = '$result->id_equipo' 
                                        AND estado_extension = 'pendiente' 
                                        LIMIT 1");
                
                $numExtensiones = $mysql->f_num($sqlExt);
                
                if ($numExtensiones > 0) {
                    $extData = $mysql->f_obj($sqlExt);
                    if ($extData && property_exists($extData, 'token')) {
                        $btn_extension = "<button class='btn btn-warning btn-sm btn-extension' data-token='$extData->token' data-toggle='modal' data-target='#modalExtension'><i class='fas fa-clock'></i> Gestionar Extensión</button>";
                    } else {
                        $btn_extension = ''; // O valor por defecto seguro
                    }
            ...
            //if ($id_usuario_sesion == 1 ) {
                $sqlX2 = $mysql->query("SELECT * FROM equipo_prestamo WHERE id_equipo ='$result->id_equipo' AND estado = 'prestado' LIMIT 1");
                $resultX2 = $mysql->f_obj($sqlX2);

                if (!empty($resultX2) && !empty($resultX2->fecha_propuesta_extension)) {
                    $color_fecha = " style='color:#4169e1;'";

                    $prestado_a .= " <span $color_fecha> <br> Fecha extensión: <br> $resultX2->fecha_propuesta_extension </span> ";
                }
            //}
        }
    }

    //error_log("Datos del botón: token={$result->token}, nombre={$result->prestado_a_nombre}, id={$result->prestado_a_id_usuario}");
