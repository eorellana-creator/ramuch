...
while($result = $mysql->f_obj($sql)) {
    ...
    if ($result->prestado_a_id_usuario > 0) {
        ...
        if ($estado_solicitud == 'solicitado') {
            ...
            // LÓGICA MODIFICADA PARA MANEJAR PRIMERA Y SEGUNDA EXTENSIÓN
            $extensionData = null;
            $extensionToken = null;
            $extensionFecha = null;
            
            // Primero verificar si hay extensión2 pendiente
            ...
            if ($mysql->f_num($sqlExt2) > 0) {
                // Hay segunda extensión pendiente
                ...
                // Si no hay segunda extensión, verificar primera extensión
            ...
            // Mostrar botón de gestión de extensión si hay alguna pendiente
            ...
            // Mostrar fecha de extensión activa
            if ($id_usuario_sesion == 1) {
                ...
                if (!empty($resultX2)) {
                    // Verificar si hay fecha de segunda extensión
                    if (!empty($resultX2->fecha_propuesta_extension2)) {
                        $color_fecha = " style='color:#4169e1;'";
                        $prestado_a .= " <span $color_fecha> <br> Fecha extensión 2: <br> $resultX2->fecha_propuesta_extension2 </span> ";
                    } 
                    // Si no hay segunda extensión, mostrar primera extensión
                    elseif (!empty($resultX2->fecha_propuesta_extension)) {
                        $color_fecha = " style='color:#4169e1;'";
