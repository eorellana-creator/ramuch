    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group" id="fechaExtensionGroup">
                    ...
                    <div class="input-group mb-2">
                        ...
                        <div class="input-group-append">
                            <button class="btn btn-outline-secondary" type="button" onclick="restaurarFechaOriginalEquipo()">
                ...
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="aplicarATodosEquipos">
                    <label class="form-check-label" for="aplicarATodosEquipos">Aplicar a todos los equipos del usuario</label>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
...
function restaurarFechaOriginalEquipo() {
...
function confirmarGestionExtensionEquipo() {
    console.log('=== DEBUG: INICIANDO confirmarGestionExtensionEquipo ===');
    ...
    const fechaExtension = document.getElementById('fechaExtension').value; // ← Este es el campo correcto
    const aplicarATodos = document.getElementById('aplicarATodosEquipos').checked;

    console.log('DEBUG - Datos capturados:', {
        token: token,
        accion: accion,
        motivo: motivo,
        fechaExtension: fechaExtension, // ← Usar este nombre
    ...
    if (accion === 'aprobar' && !fechaExtension) {
        alert('Por favor seleccione una fecha para la extensión');
        return;
    }
    ...
    // CORREGIR: Enviar con el nombre correcto "fecha_extension"
    const params = new URLSearchParams();
    params.append('token', token);
    params.append('accion', accion);
    params.append('motivo', motivo);
    params.append('fecha_extension', fechaExtension); // ← NOMBRE CORRECTO
    params.append('aplicar_a_todos', aplicarATodos ? '1' : '0');

    console.log('DEBUG - Parámetros enviados:', params.toString());

    fetch('components/equipo/models/procesar_extension.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
...
function mostrarMensajeElegante(tipo, mensaje) {
    // Crear contenedor de mensajes si no existe
    if (!$('#mensajes-flotantes').length) {
        $('body').append('<div id="mensajes-flotantes" style="position: fixed; top: 20px; right: 20px; z-index: 9999;"></div>');
    }
    ...
    $('#mensajes-flotantes').append(mensajeHTML);
...
$(document).ready(function() {
    ...
    // Configuración del modal de extensión - ACTUALIZADO
