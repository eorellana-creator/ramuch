...
if($token==""){
	...
	$sql 	= $mysql->query("INSERT INTO cuenta_maestra (id_usuario_sistema, nombre_usuario_sistema, id_usuario_movimiento, nombre,  fecha, tipo, sub_cuenta, glosa, observacion, medio, id_transaccion, documento_respaldo, monto, estado, token)
												VALUES ( '$id_usuario', '$nombre_usuario_sistema',     '$id_participante',       '$nombre', '$fecha', 'ingreso', 'otros','$glosa','$observacion','$medio','$aleatorio $hoy $id_usuario $monto','','$monto','activo', '$token_nuevo'  ) ;");
	...
	if($token!=""){
	$sql 	= $mysql->query("UPDATE cuenta_maestra SET  glosa='$glosa', fecha='$fecha' , monto='$monto', medio='$medio', observacion='$observacion' WHERE token ='$token';");
}
}
